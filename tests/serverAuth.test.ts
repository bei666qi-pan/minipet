...
import { hashPassword, signToken, verifyPassword, verifyToken } from "../apps/backend/src/auth";

describe("MiniPet backend auth", () => {
  it("signs scoped tokens and verifies password hashes", () => {
    ...
    const hash = hashPassword("local-pass", "salt");
    expect(verifyPassword("local-pass", hash)).toBe(true);
